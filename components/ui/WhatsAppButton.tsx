 ...
 *
 * Floating button that appears globally on all pages
...
interface WhatsAppButtonProps {
  message?: string;
  className?: string;
  fixed?: boolean; // If true, button will be fixed at bottom-right
}
...
export default function WhatsAppButton({
  ...
  return (
    ...
    >
      {/* WhatsApp Icon */}
      <svg
        className={fixed ? 'w-8 h-8' : 'w-6 h-6 mr-2'}
        fill="currentColor"
        viewBox="0 0 24 24"
      ...
      {!fixed && <span>Contactar por WhatsApp</span>}
    </button>
  );
}
